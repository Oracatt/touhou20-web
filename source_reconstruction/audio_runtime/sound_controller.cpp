...
int SoundInf::poll() {
    ...
    if(stream && stream->playing && !stream->busy) {
        stream->busy=1;
        for(unsigned count=0;count<16&&th20_web_audio_notification_count(stream->buffer(0),0)>0;++count){
            const auto previous=stream->next_write;
            stream->handle_notification(true);
            if(stream->next_write==previous)break;
            th20_web_audio_notification_count(stream->buffer(0),1);
        }
        stream->busy=0;
    }
#endif
