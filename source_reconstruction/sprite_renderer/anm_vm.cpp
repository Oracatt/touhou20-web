...
std::int32_t execute_animation(Animation& a){
    ...
    if(a.fields_550[6]){
        ...
        // WebAssembly uses Clang's Itanium vtable layout, which has two
        // destructor entries before update(), so dispatch through the C++
        // interface and let the compiler select the browser ABI slot.
        auto* callback=reinterpret_cast<AnimationCallback*>(a.fields_550[6]);
