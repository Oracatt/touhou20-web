...
void finish_curve(Controller& c,D3DPRIMITIVETYPE type,std::uint32_t count,std::uint32_t vertices,bool depth){if(depth){e::disable_depth_write();dispatch_environment::set_render_state(23,8);}c.unknown_cached_e0e=1;select_texture_combine(c,e::device(),2);e::device().SetFVF(0x44);e::device().DrawPrimitiveUP(type,count,c.colored_write,20);c.colored_write=reinterpret_cast<Vertex20*>(reinterpret_cast<std::uint32_t>(c.colored_write)+vertices*20);++c.draw_calls;}
...
std::int32_t p43b960(Controller& c,float x,float y,float radius,float angle,std::uint32_t raw_count,std::uint32_t primary,std::uint32_t secondary){
    ...
    e::disable_depth_write();dispatch_environment::set_render_state(23,8);c.unknown_cached_e0e=1;select_texture_combine(c,e::device(),2);e::device().SetFVF(0x44);e::device().DrawPrimitiveUP(D3DPT_TRIANGLEFAN,raw_count,c.colored_write,20);c.colored_write=reinterpret_cast<Vertex20*>(reinterpret_cast<std::uint32_t>(c.colored_write)+(raw_count+2)*20);++c.draw_calls;return 0;
}
