...
#else
        std::lock_guard<std::recursive_mutex> lock(runtime::shared_locks().slot(6));runtime::detach_worker(o.worker);o.worker.close_requested.store(false,std::memory_order_seq_cst);o.worker.thread=std::jthread([&o]{read_music_comments(o);});
#endif
    }
